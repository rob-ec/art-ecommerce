...
<template>
  <div class="w-fit cursor-pointer" @click="openCart">
    <i class="pi pi-shopping-cart text-xl text-palette-brown" />
  </div>
  <div class="fixed inset-0 z-50 flex flex-col min-h-screen overflow-hidden bg-transparent" v-if="showCart">
    <div class="absolute inset-0 w-full h-full min-h-[120vh] bg-black/40" @click="closeCart"></div>
    <aside class="absolute top-0 right-0 bottom-0 bg-palette-gray-light flex flex-col p-5 w-full max-w-125 overflow-y-auto">
      <h1 class="flex items-center gap-4 text-lg mb-10 text-palette-brown font-bold uppercase">
      ...
      <section class="flex flex-col gap-2.5 h-fit">
        ...
        <div v-if="productList.length > 0">
          <RouterLink
            to="/checkout"
            class="mt-4 inline-block w-full text-center bg-palette-green text-palette-white py-3 rounded-lg font-bold uppercase hover:bg-palette-green/90 transition-colors"
          >
            Finalizar Compra
          </RouterLink>
        </div>
